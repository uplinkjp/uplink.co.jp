<?php get_header('film'); ?>
	...
	<div id="container" class="clearfix film list films">
		...
		<div id="maincontents">
			<nav id="pan">
				<p><a href="/">ホーム</a> &#8250; <a href="/film/">配給作品</a> &#8250; <em><?php single_term_title('', true); ?></em></p>
			</nav>
			
			<!-- posts str -->
			<section class="posts">
				<header class="contents-title">
					<h1>配給作品 - <?php single_term_title('', true); ?></h1>
					
					<?php if(is_tax('film-tag','rental')): ?>
					<p>料金など詳細については<a href="/film/rental/" target="_blank">自主上映のご案内</a>のページをご覧ください</p>
					<?php endif; ?>
				</header>
				...
				<article class="clearfix post">
					<header>
						...
						<?php if ( post_custom('film_main_1') ) : ?><p class="print"><?php $date = post_custom("film_main_1"); echo date('劇場公開日：Y年n月j日', strtotime($date)); ?></p><?php endif; ?>
						<?php echo get_the_term_list( $post->ID, 'film-material', '<ul class="material clearfix"><li>', '</li><li>', '</li></ul>' ); ?>
						<?php if ( post_custom('film_txt_3') ) : ?><p class="credit"><?php echo nl2br(get_post_meta($post->ID, 'film_txt_3', true)); ?></p><?php endif; ?>
					</header>
					<figure>
